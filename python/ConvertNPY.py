...
for img in os.listdir(train_esca_data_dir):
    if first_time:
        ...
        ytrain = np.concatenate((ytrain,np.array([[1.]])), axis=0)     # 1 is esca
...
for img in os.listdir(train_healthy_data_dir):
    ...
    tmp = np.resize(tmp, (1, tmp.shape[0], tmp.shape[1], tmp.shape[2]))
    xtrain = np.concatenate((xtrain, tmp), axis=0)
    ytrain = np.concatenate((ytrain,np.array([[0.]])), axis=0)         # 0 is healthy 
...
for img in os.listdir(test_esca_data_dir):
    if first_time:
        ...
        ytest = np.concatenate((ytest,np.array([[1.]])), axis=0)     # 1 is esca
...
for img in os.listdir(test_healthy_data_dir):
    ...
    tmp = np.resize(tmp, (1, tmp.shape[0], tmp.shape[1], tmp.shape[2]))
    xtest = np.concatenate((xtest, tmp), axis=0)
    ytest = np.concatenate((ytest,np.array([[0.]])), axis=0)         # 0 is healthy 
...
PATH_DATASET_IMAGE = './processed_esca_dataset/validation'
...
PATH_DATASET = './NPY_Dataset/validation'
...
for img in os.listdir(validation_esca_data_dir):
    if first_time:
        ...
        yvalidation = np.concatenate((yvalidation,np.array([[1.]])), axis=0)     # 1 is esca
...
for img in os.listdir(validation_healthy_data_dir):
    ...
    tmp = np.resize(tmp, (1, tmp.shape[0], tmp.shape[1], tmp.shape[2]))
    xvalidation = np.concatenate((xvalidation, tmp), axis=0)
    yvalidation = np.concatenate((yvalidation,np.array([[0.]])), axis=0)         # 0 is healthy 
